import { accessSync, chmodSync, constants as fsConstants, copyFileSync, mkdirSync, writeFileSync } from 'node:fs';
...
/**
 * Picks between the system-wide `/Applications` that Finder shows by default and the per-user
 * `~/Applications` fallback. `/Applications` is `root:admin`-owned and group-writable on the
 * common single-admin-user Mac (the overwhelming majority of hobbyist setups), so this unelevated,
 * un-installer-run process can usually write there directly; `~/Applications` — invisible in
 * Finder's default view — only exists to cover managed/restricted accounts where it can't.
 */
export function resolveMacAppsDir(homeDir: string, systemAppsDir: string = '/Applications'): string {
  try {
    accessSync(systemAppsDir, fsConstants.W_OK);
    return systemAppsDir;
  } catch {
    return join(homeDir, 'Applications');
  }
}

/** Writes the whole \`.app\` bundle to the resolved Applications dir — see resolveMacAppsDir. */
