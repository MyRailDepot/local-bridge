...
async function bootstrap(): Promise<void> {
  ...
  try {
    const creds = await ensureCredentials({
      // Always the fixed install dir, never process.cwd(): on the very first (npx, ephemeral)
      // run cwd is wherever the user happened to be when they ran the command, and a stray .env
      // full of secrets left behind there serves no purpose once self-install below persists the
      // real copy here moments later. On a launch from the installed shortcut cwd already equals
      // this same directory (the launcher cd's there first), so this is a no-op change for that case.
  ...
  // install, it only refreshes it in the background for next time. selfInstallIfNeeded already
  // catches everything it does internally and never rejects — the .catch() here is belt-and-
  // braces only, so a future edit that reintroduces a throwing statement ahead of its own try
  // block can never turn into an unhandled rejection that kills an already-serving bridge.
  ...
  }).catch(() => { /* never fatal — selfInstallIfNeeded reports its own failures */ });
}
